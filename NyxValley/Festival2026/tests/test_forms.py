...
class TestFormulariosFestival(TestCase):
    ...
    # ─────────────────────────────────────────────────────────────
    #  4. Pruebas para ParqueForm
    # ─────────────────────────────────────────────────────────────
    def test_parque_form_datos_validos(self):
        """Verifica que el formulario de parque sea válido con datos correctos y completos."""
        datos = {
            'nombre': 'Parque Nacional Amecameca',
            'direccion': 'EdoMex, México',
            ...
            'tiene_cabanas': True,
            'tiene_camping': True,
            'servicios': 'Zona de fogatas regulada.'
        }
        form = ParqueForm(data=datos)
        self.assertTrue(form.is_valid())

    def test_parque_form_horario_invalido(self):
        """Verifica que falle si la hora de apertura es posterior o igual a la de cierre."""
        datos = {
            'nombre': 'Parque Horarios Corruptos',
            'direccion': 'CDMX',
            'horario_apertura': time(17, 0), # 5:00 PM
        ...
        self.assertIn('horario_apertura', form.errors)
        self.assertIn('La hora de apertura debe ser anterior a la de cierre', form.errors['horario_apertura'])

    def test_parque_form_error_sin_ningun_hospedaje(self):
        """Verifica que se rechace el parque si no ofrece ni cabañas ni zona de camping."""
        datos = {
            'nombre': 'Parque Sin Hospedaje',
            ...
            'tiene_cabanas': False,
            'tiene_camping': False,
            'servicios': 'Solo visitas guiadas diurnas'
        }
        form = ParqueForm(data=datos)
        self.assertFalse(form.is_valid())
        
        self.assertIn('El parque debe ofrecer al menos zona de camping', form.non_field_errors())